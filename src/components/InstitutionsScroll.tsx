  ...
  return (
    <section className="py-12 bg-white overflow-hidden">
      ...
      <div className="relative w-full">
        <ScrollVelocityContainer className="py-8 bg-gradient-to-r from-[#f8f9fa] via-white to-[#f8f9fa] overflow-visible">
          <ScrollVelocityRow baseVelocity={1} direction={-1} className="py-4">
            {healthInstitutions.map((institution, index) => (
              <motion.div
                key={index}
                className="mx-4 px-6 py-4 bg-white rounded-lg shadow-md border border-gray-100 flex items-center transform-gpu"
                whileHover={{ 
                  scale: 1.05, 
                  rotateY: 5,
                  z: 20,
              ...
              >
                <span className="text-lg font-medium text-gray-800">{institution}</span>
              </motion.div>
            ))}
          </ScrollVelocityRow>
          
          <ScrollVelocityRow baseVelocity={1.2} direction={1} className="py-4">
